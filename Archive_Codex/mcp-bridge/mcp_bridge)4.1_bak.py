...
# Bridge AI identifier for responses
BRIDGE_AI_ID = os.getenv('CODEX_BRIDGE_AI_ID', 'codex')
...
def chat_with_codex():
    ...
    if not _ai_rate_limit_check(requester_ai_id, '/chat-with-codex', CHAT_RPM_LIMIT):
    ...
    if session_id:
        ...
        msg_user = {"from": "user", "ai_id": requester_ai_id, "content": user_message, "ts": _now_iso()}
    ...
    try:
        ...
        if sess and content:
            msg_ai = {"from": "ai", "ai_id": BRIDGE_AI_ID, "content": content, "ts": _now_iso()}
...
def chat_with_codex_stream():
    ...
    if not _ai_rate_limit_check(requester_ai_id, '/chat-with-codex/stream', CHAT_RPM_LIMIT):
        def denied():
    ...
    def event_stream():
        ...
        try:
            ...
            if sess and full:
                msg_ai = {"from": "ai", "ai_id": BRIDGE_AI_ID, "content": full, "ts": _now_iso()}
                sess.messages.append(msg_ai)
...
<html>
  <head>
    ...
    <style>
      ...
      select, button, input[type=text] {{ background:#0f172a; color:#e6edf3; border:1px solid #334155; padding:6px 8px; border-radius:6px; }}
      ...
      .msg.ai {{ background: #0d3b66; border-top-right-radius: 4px; }}
    ...
    <script>
      ...
      function renderMessages(items) {{
        ...
        (items || []).forEach(m => {{
          ...
          const meta = document.createElement('div'); meta.className = 'meta ' + (m.from === 'ai' ? 'right' : 'left');
          meta.textContent = `${{m.ai_id ? m.ai_id : (m.from === 'ai' ? 'codex' : 'user')}} • ${{m.ts || ''}}`;
          const bubble = document.createElement('div'); bubble.className = 'msg ' + (m.from === 'ai' ? 'ai right' : 'user left'); bubble.textContent = m.content || '';
...
<html>
  <head>
    <meta charset=\"utf-8\" />
    <meta name=\"viewport\" content=\"width=device-width, initial-scale=1\" />
    <title>Codex Chat Monitor — Live</title>
    <style>
      body {{ font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif; margin: 0; background: #0b0d10; color: #e6edf3; }}
      header {{ padding: 12px 16px; background: #11161c; border-bottom: 1px solid #1f2937; display:flex; align-items:center; gap:8px; flex-wrap: wrap; }}
  ...
  <body>
    ...
    <div id=\"compose\">
      ...
      <input id=\"msgBox\" type=\"text\" placeholder=\"Type a message…\" />
      <label style=\"display:flex; align-items:center; gap:6px; font-size:12px; opacity:.9\"> <input id=\"autoReply\" type=\"checkbox\" checked /> Ask Codex to reply</label>
      <label style=\"display:flex; align-items:center; gap:6px; font-size:12px; opacity:.9\"> <input id=\"liveToggle\" type=\"checkbox\" /> Live</label>
    ...
    <script>
      ...
      const chat = document.getElementById('chat');
      const stats = document.getElementById('stats');
      const liveCb = document.getElementById('liveToggle');
      ...
      async function sendMessage() {{
        ...
        btn.disabled = true; btn.textContent = 'Sending…';
        try {{
          if (auto && liveCb.checked) {{
            await streamReply(fromId, text);
          }} else if (auto) {{
            await fetch('/chat-with-codex', {{
              method: 'POST',
              headers: {{ 'Content-Type': 'application/json', 'X-AI-Id': fromId }},
      ...
      async function streamReply(fromId, text) {{
        // Append a local user bubble for immediate feedback
        const urow = document.createElement('div'); urow.className = 'row';
        const umeta = document.createElement('div'); umeta.className = 'meta left'; umeta.textContent = `${{fromId}} • now`;
        const ubub = document.createElement('div'); ubub.className = 'msg user left'; ubub.textContent = text;
        urow.appendChild(umeta); urow.appendChild(ubub); chat.appendChild(urow);
        // Append an AI bubble that we will fill as tokens arrive
        const arow = document.createElement('div'); arow.className = 'row';
        const ameta = document.createElement('div'); ameta.className = 'meta right'; ameta.textContent = `codex • streaming`;
        const abub = document.createElement('div'); abub.className = 'msg ai right'; abub.textContent = '';
        arow.appendChild(ameta); arow.appendChild(abub); chat.appendChild(arow);
        chat.scrollTop = chat.scrollHeight;

        const resp = await fetch('/chat-with-codex/stream', {{
          method: 'POST',
          headers: {{ 'Content-Type': 'application/json', 'X-AI-Id': fromId }},
          body: JSON.stringify({{ message: text, session_id: current, debug: false }})
        }});
          ...
          while ((idx = buffer.indexOf(\n\n)) !== -1) {{
            ...
            try {{
              ...
              if (ev.type === 'token' && typeof ev.content === 'string') {{
                abub.textContent += ev.content;
                chat.scrollTop = chat.scrollHeight;
              }} else if (ev.type === 'error') {{
                ameta.textContent = `codex • error`;
              }} else if (ev.type === 'response_end') {{
                ameta.textContent = `codex • done`;
              }}
            }} catch {{}}
          }}
        }}
